...
class Puffin(nn.Module):
    def __init__(self, use_cuda=False):
        ...
        model_path = os.path.join(current_dir, "resources", "puffin.pth")
...
if __name__ == "__main__":
    ...
    doc = """
    Puffin outputs transcription initiation signal prediction for the input genome sequence.

    Usage:
        puffin.py coord [options] <coordinate>
        puffin.py sequence [options] <fasta_file_path>
        puffin.py region [options] <tsv_file>

    Options:
        -h --help                   Show this screen.
        --no_interpretation         Leave only Puffin prediction
        --targeti=<targeti>         Experimental method target
                                    (FANTOM_CAGE|ENCODE_CAGE|ENCODE_RAMPAGE|GRO_CAP|PRO_CAP)
        --both_strands              Generate prediction for the opposite strand
        --cuda                      Use CUDA to generate the prediction
        --output=<DIR>              Output directory [default: ./]
        --genome=<FILE>             Genome FASTA file path
    """
    ...
    output = arguments["--output"] if arguments["--output"] else 'temp'
    os.makedirs(output, exist_ok=True)

    if arguments["--genome"]:
        genome_path = arguments["--genome"]
    else:
        sys.exit("Error: please provide --genome <genome_path>")
    ...
    for seq_bp, name in zip(seq_list, name_list):
        ...
        if arguments["--both_strands"]:
            ...
            df.to_csv(f"{output}/{name}" + "_rev_strand.csv")
