...
class MappingToReference:
    def __init__(self, 
                 error_sites_ratio=0.02,           # Using _ref parameter
                 little_exon_jump_ratio=0.1,       # Using _ref parameter
                 Nonlittle_exon_jump_ratio=0.1,    # Using _ref parameter
                 fake_exon_group_freq_ratio=0.2,   # Using _ref parameter
                 little_exon_bp=30,
        ...
        """
        # Using _ref suffix parameters (for reference-based scenarios)
    ...
    def filter_groups(self, df, ref_anno):
        """
        Filter groups based on reference annotation and frequency.
        """
    ...
    def correct_ssc(self, row, dict_correct, df1):
        """
        Correct SSC based on dictionary mapping.
        """
        FSM_SSC = df1[
            (df1['Chr'] == row['Chr']) & 
    ...
    def correct_for_chr(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Correct transcripts for a chromosome.
        """
        if df.empty:
            ...
            for row in df_nnc_nic.itertuples():
                ...
                if row.category == 'NIC' and ref_little_exon_paths:
                    ...
                    # 2
                        ...
                        if row.frequency / group_freq < self.mismatch_error_sites_groupfreq_ratio:
                        ...
                        if len(duplicates) == 1:
                            ...
                            if strand == '+':
                                ...
                                if (len(np.unique(tail_sites)) == 1 and 
                                    row.frequency / group_freq < self.mismatch_error_sites_groupfreq_ratio and
    ...
    def correct_transcripts(self, df, ref_anno):
        """
        Correct transcripts using reference annotation.
        """
    ...
    def map_to_reference(self, df_raw, df, ref_anno):
        """
        Main mapping function, similar to the old version's anno_process.
        """
        ...
        df = self.correct_transcripts(df, ref_anno)
        df['frequency'] = df['TrStart_reads'].apply(len)
